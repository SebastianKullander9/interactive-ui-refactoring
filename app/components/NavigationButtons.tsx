...
export default function NavigationButtons({ lessons, handlePrev, handleNext, pageIndex, setPageIndex, phase}: NavigationButtonsProps) {
    const [reducedLessons, setReducedLessons] = useState<Lesson[]>([]);
    
    const getCurrentThirdOfLessons = () => {
        ...
        const currentThird = lessons.slice(start, end);

        return currentThird;
    }

    useEffect(() => {
        setReducedLessons(getCurrentThirdOfLessons)
    }, [pageIndex])
